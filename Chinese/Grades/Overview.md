# 开发者分级

> 分级是一种根据开发人员的技能和经验进行分类的方法。通过分级，可以了解如何区分任务的难度以及处理它们所需的预期技能集。

现在，您可以遇到以下常见的评分方法。可以标记以下级别：

- 初级
- 中级
- 高级

每个公司都有自己对开发者分级的理解，也会为不同等级声明不同的技能和职责。通常会遇到以下情况：您可能会在一家公司晋升为高级角色，但在另一家公司中被评估为中级开发者。尽管如此，仍然可以针对每个等级引入共同期望。我们将使用前面描述的简化分类来描述每个等级。

**示例：** 您可以查看此网站以了解不同公司的职级体系：[levels.fyi](https://www.levels.fyi/)
![定位](Source/GradeTable.PNG "GradeTable")

## 级别说明

您可以阅读这些文章来了解每个级别及其共同期望：

- :alien: [Pre-Junior C++](PreJunior.md)
- :yum: [Junior C++](Junior.md)
- :sunglasses: [Middle C++](Middle.md)
- :smiling_imp: [Senior C++](Senior.md)

---

[**返回主页**](../README.md)
