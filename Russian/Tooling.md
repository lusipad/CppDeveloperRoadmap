...
## :keyboard: Языковые сервисы и форматирование

* :arrow_forward: **clangd**

    Сайт: [https://clangd.llvm.org/](https://clangd.llvm.org/)

    Языковой сервер для C и C++, который обеспечивает автодополнение, навигацию, диагностику и рефакторинг во многих редакторах, включая Visual Studio Code и Vim/Neovim.

* :arrow_forward: **clang-format**

    Сайт: [https://clang.llvm.org/docs/ClangFormat.html](https://clang.llvm.org/docs/ClangFormat.html)

    Форматтер исходного кода, который помогает командам автоматически поддерживать единый стиль. Часто используется локально и в проверках CI.

* :arrow_forward: **clang-tidy**

    Сайт: [https://clang.llvm.org/extra/clang-tidy/](https://clang.llvm.org/extra/clang-tidy/)

    Инструмент статического анализа и линтинга из проекта LLVM. Он помогает находить опасные паттерны, возможности модернизации, проблемы читаемости и нарушения стиля.
...
* :arrow_forward: **CMake**
    ...
    Кроссплатформенная система автоматизации сборки приложения из исходного кода. Генерирует необходимые артефакты для последующей сборки приложения на целевой платформе. На текущий момент считается "стандартным" инструментом для сборки многих C и C++ проектов.

* :arrow_forward: **CMake Presets**

    Сайт: [https://cmake.org/cmake/help/latest/manual/cmake-presets.7.html](https://cmake.org/cmake/help/latest/manual/cmake-presets.7.html)

    Возможность CMake для хранения типовых настроек конфигурации, сборки, тестирования и упаковки в `CMakePresets.json`. Помогает командам разделять воспроизводимые локальные и CI сценарии сборки.
...
* :arrow_forward: **Conan**
    ...
    Пакетный менеджер, а также менеджер зависимостей для организации C++ библиотек и фреймворков. Современные сценарии Conan 2 хорошо интегрируются с CMake и часто применяются, когда команде нужен явный контроль над бинарными пакетами, профилями и настройками сборки.

* :arrow_forward: **vcpkg**

    Сайт: [https://learn.microsoft.com/en-us/vcpkg/](https://learn.microsoft.com/en-us/vcpkg/)

    Стоимость: бесплатно

    Кроссплатформенный пакетный менеджер для C и C++. Manifest mode позволяет проекту описывать зависимости в `vcpkg.json`, что упрощает воспроизведение зависимостей на машинах разработчиков и в CI.
...
## :mag: Анализаторы кода и runtime-проверки
...
* :arrow_forward: **Sanitizers**
    ...
    Сайт: [https://clang.llvm.org/docs/](https://clang.llvm.org/docs/)

    Runtime-проверки вроде AddressSanitizer, UndefinedBehaviorSanitizer, ThreadSanitizer и MemorySanitizer помогают находить проблемы с памятью, неопределенным поведением и конкурентностью во время разработки и в CI.

* :arrow_forward: **CodeQL**

    Сайт: [https://docs.github.com/en/code-security/code-scanning/introduction-to-code-scanning/about-code-scanning-with-codeql](https://docs.github.com/en/code-security/code-scanning/introduction-to-code-scanning/about-code-scanning-with-codeql)

    Семантический движок анализа кода, который можно использовать в GitHub code scanning для поиска проблем безопасности и корректности в C и C++ проектах.

## :test_tube: Тестирование и автоматизация

* :arrow_forward: **GoogleTest**

    Сайт: [https://google.github.io/googletest/](https://google.github.io/googletest/)

    Широко используемый фреймворк для тестирования и мокирования в C++. Частый выбор для unit-тестов в коммерческих и open source проектах.

* :arrow_forward: **Catch2**

    Сайт: [https://github.com/catchorg/Catch2](https://github.com/catchorg/Catch2)

    Фреймворк тестирования C++, удобный для небольших проектов, примеров и читаемых тестовых сценариев.

* :arrow_forward: **libFuzzer**

    Сайт: [https://llvm.org/docs/LibFuzzer.html](https://llvm.org/docs/LibFuzzer.html)

    Coverage-guided fuzzing движок для поиска падений и пограничных случаев через автоматическую генерацию большого числа входных данных.

* :arrow_forward: **GitHub Actions**

    Сайт: [https://docs.github.com/en/actions](https://docs.github.com/en/actions)

    Распространенная CI платформа для сборки, тестирования, линтинга и упаковки C++ проектов при каждом изменении.
...
* :arrow_forward: **Atlassian SourceTree**
    ...
    Отличная бесплатная альтернатива для работы с git через графический интерфейс. Не уступает по функционалу SmartGit, за исключением отсутствия собственного редактора разрешения конфликтов. Это легко исправляется интеграцией с Visual Studio Code или любого другого редактора, который умеет сравнивать файлы между собой. Во всем остальном дублирует функциональность SmartGit: кроссплатформенный, поддерживает интеграцию с публичными репозиториями: GitHub, Bitbucket, GitLab и т.д.
